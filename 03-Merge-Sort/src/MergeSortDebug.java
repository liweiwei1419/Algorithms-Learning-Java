...
public class MergeSortDebug {
    ...
    private void mergeSort(int[] arr, int left, int right, int debug_depth) {
        System.out.println(printDepth(debug_depth) + "sort : " + left + " " + right);
        if (left >= right) {
            return;
        }
        int mid = left + (right - left) / 2;
        mergeSort(arr, left, mid, debug_depth + 1);
        mergeSort(arr, mid + 1, right, debug_depth + 1);
        if (arr[mid] <= arr[mid + 1]) {
            return;
        }
        mergeOfTwoSortArray(arr, left, mid, right);
        System.out.println(printDepth(debug_depth) + "merge: " + left + " " + right);
    }
